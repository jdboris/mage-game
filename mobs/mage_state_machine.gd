...
export(NodePath) onready var casting = get_node(casting) as Node
var CastingCursor := preload("res://ui/casting_cursor.tscn")
...
func _unhandled_input(event: InputEvent):
	if event.is_action_pressed("fire_rune"):
		...
		# TODO: Do this right
#		if MouseCursor.cursor is CastingCursor:
...
#		if MouseCursor.cursor is CastingCursor:
		if not MouseCursor.cursor:
			MouseCursor.set_mouse_cursor(CastingCursor)


func cast_spell(spell, target_pos: Vector2):
	
	if commands.queue.size() > 1 or spell.cooldown_timer.time_left:
		return false
	
#	var cast_time_left = current_state.time_left if current_state == casting else 0
#
