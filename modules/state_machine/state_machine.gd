...
func _change_state(state: Node = null, args := {}):
	assert(
		state || current_state.is_reversible,
		"Error: new state is null, but current_state is not reversible."
	)
	...
	if state == null:
		assert(
			states_stack.size() > 1,
			(
				"Error: Attempted to return to previous from State ('"
				+ current_state.get_path()
				+ "') with no more States on the stack. Is is_reversible set to true?"
			)
		)
